...
logging.basicConfig(filename=save_path + 'RGBDNewNet.log',
                    format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]', level=logging.INFO, filemode='a',
                    datefmt='%Y-%m-%d %I:%M:%S %p')
logging.info("NewNet-Train_4_pairs")
...
if opt.load_swin is not None:
    model.load_pre(opt.load_swin)
    print('load Swin model from ', opt.load_swin)
...
logging.info(
    'epoch:{};lr:{};batchsize:{};trainsize:{};clip:{};decay_rate:{};load:{};save_path:{};decay_epoch:{}'.format(
        opt.epoch, opt.lr, opt.batchsize, opt.trainsize, opt.clip, opt.decay_rate, opt.load_swin, save_path,
        opt.decay_epoch))
    ...
    try:
        for i, (images, gts, depth, edge) in enumerate(train_loader, start=1):
            ...
            if i % 100 == 0 or i == total_step or i == 1:
                ...
                print(
                    '{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR:{:.7f}||sal_loss:{:4f} sal_loss1:{:4f} edge_loss:{:4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step,
                           optimizer.state_dict()['param_groups'][0]['lr'], sal_loss.data, sal_loss1.data,
                           edge_loss.data))
                logging.info(
                    '#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR:{:.7f},  sal_loss:{:4f}  sal_loss1:{:4f} edge_loss:{:4f} ||, mem_use:{:.0f}MB'.
                    format(epoch, opt.epoch, i, total_step, optimizer.state_dict()['param_groups'][0]['lr'],
                           sal_loss.data, sal_loss1.data, edge_loss.data, memory_used))
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + 'NewNet_epoch_best.pth')
